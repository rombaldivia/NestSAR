...
'''
    ...
    print("M4-PHASEPATH-JITTER-UNIFORM-T16 | ISOLATED TPU RUNTIME PROBE", flush=True)
    print("=" * 120, flush=True)
    ...
    if backend != "tpu" or count != EXPECTED_DEVICES:
        raise RuntimeError(
            f"Expected TPU/8; got backend={backend}, count={count}"
        )
...
def main() -> int:
    ...
    run_child(
        "experiments.m4_phasepath_jitter_uniform_t16.preflight_tpu8"
    )

    print(
        "PREFLIGHTS PASSED — STARTING PHASEPATH JITTER + UNIFORM-FUSION TRAINING",
        flush=True,
    )

    run_child(
        "experiments.m4_phasepath_jitter_uniform_t16.train_tpu",
        "--protocol", "both",
        "--epochs", "60",
        "--patience", "12",
        "--batch-size", "256",
        ...
        "--progress-every", "5",
        "--audit-first",
        "--outdir", "/kaggle/working/NestSAR_M4_PhasePath_JitterUniform_T16_TPU",
    )
    return 0
