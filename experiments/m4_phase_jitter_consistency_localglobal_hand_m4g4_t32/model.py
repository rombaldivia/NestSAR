...
EXPECTED_PARAMS_D32 = BASELINE_PARAMS + HAND_EXTRA_PARAMS_D32
...
class HandM4G4LiteT32(nn.Module):
    ...
    @nn.compact
    def __call__(self, hand_x: jnp.ndarray, training: bool = False) -> Mapping[str, jnp.ndarray]:
        if hand_x.shape[1] != HAND_FRAMES or hand_x.shape[2] != HAND_FEATURES:
            raise ValueError(
                f"Expected hand input [B,{HAND_FRAMES},{HAND_FEATURES}], got {hand_x.shape}"
            )
...
class M4LocalGlobalHandM4G4T32(nn.Module):
    ...
    def __call__(
        ...
        logits = main_logits + self.hand_residual_scale * hand["logits"]
