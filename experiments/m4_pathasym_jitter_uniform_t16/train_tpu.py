...
"""Path-Asymmetry T16 + training-only segment jitter + fixed uniform fusion.

Clean representation ablation on top of the successful Jitter+Uniform model.
Keep the validated total-path statistic and add only a dimensionless early-vs-late
path asymmetry signal:

    asym = (path_A - path_B) / (path_A + path_B + eps)
...
Per segment / person / joint token:
  ...
  total absolute path xyz
  early-vs-late path asymmetry xyz

Geometric channels are RMS-normalized exactly as before. The asymmetry channels
are dimensionless and are therefore NOT divided by the geometric RMS.

Training uses canonical and +/-1-frame boundary-jittered precomputed views and
chooses between them per sample each epoch, exactly as Jitter+Uniform.
Validation/inference use canonical segmentation only. No consistency loss is
used in this branch so the representation gain is isolated cleanly.
"""
...
def pathasym_tokens_from_bounds(keypoints: np.ndarray, bounds) -> np.ndarray:
    ...
    for i, (s, e) in enumerate(bounds):
        ...
        if len(seg) >= 2:
            ...
            total_path = path_a + path_b
            path_asym = (path_a - path_b) / (total_path + ASYM_EPS)
        else:
            full_disp = np.zeros_like(pose)
            phase_a = np.zeros_like(pose)
            phase_b = np.zeros_like(pose)
            total_path = np.zeros_like(pose)
            path_asym = np.zeros_like(pose)

        tokens[i, ..., 0:3] = pose
        tokens[i, ..., 3:6] = full_disp
        tokens[i, ..., 6:9] = phase_a
        tokens[i, ..., 9:12] = phase_b
        tokens[i, ..., 12:15] = total_path
        tokens[i, ..., 15:18] = np.clip(path_asym, -1.0, 1.0)

    # Normalize only geometric channels. The asymmetry channels are dimensionless.
    nz = np.abs(x) > 1e-8
    if np.any(nz):
...
def segment_pathasym_tokens(keypoints: np.ndarray) -> np.ndarray:
    x = base.canonicalize_raw(keypoints)
    bounds = base.segment_bounds(x.shape[0], FRAMES)
    return pathasym_tokens_from_bounds(keypoints, bounds)


def jitter_pathasym_tokens(
...
class M4PathAsymUniformT16(nn.Module):
    spatial_dim: int = 24
    model_dim: int = 112
    dropout: float = 0.10
    ...
    def __call__(
        ...
        # Preserve the successful total-path feature and add only early/late asymmetry.
        joint_motion = jnp.concatenate(
            [full_disp, phase_a, phase_b, total_path, path_asym], axis=-1
        )
        ...
        parent_path = jnp.take(total_path, parents, axis=3)
        parent_asym = jnp.take(path_asym, parents, axis=3)
        ...
        bone_motion = jnp.concatenate(
            [
                ...
                phase_a - parent_a,
                phase_b - parent_b,
                jnp.abs(total_path - parent_path),
                path_asym - parent_asym,
            ],
            axis=-1,
        )
...
def build_protocol_views(
    ...
    for i, sid in enumerate(
        tqdm(
            train_ids,
            desc=f"{protocol.upper()} PathAsym train canonical+jitter",
            mininterval=0.5,
        )
    ):
        a = by_id[sid]
        kp = base.annotation_keypoints(a)
        Xcan[i] = segment_pathasym_tokens(kp)
        rng = np.random.default_rng(
            np.random.SeedSequence([seed, i, 9173])
        )
        Xjit[i] = jitter_pathasym_tokens(kp, max_shift, rng)
    ...
    for i, sid in enumerate(
        tqdm(
            val_ids,
            desc=f"{protocol.upper()} PathAsym val canonical",
            mininterval=0.5,
        )
    ):
        a = by_id[sid]
        Xva[i] = segment_pathasym_tokens(base.annotation_keypoints(a))
...
def install_overrides() -> None:
    ...
    ju.M4PhaseUniformT16 = M4PathAsymUniformT16
...
def rewrite_checkpoint_metadata(outdir: str, protocol: str) -> None:
    ...
    payload["model"] = "M4PathAsymJitterUniformT16"
    payload["representation"] = {
        ...
        "path_asymmetry": "(path_A-path_B)/(path_A+path_B+eps)",
        "path_asymmetry_eps": ASYM_EPS,
...
def main() -> None:
    ...
    ju.log(
        "Experiment: PathAsym-T16 + training-only segment jitter + "
    ...
    ju.log(
        "Representation: pose + full disp + phase-A/B disp + total path + "
        "early/late path asymmetry"
    )
    ju.log(
