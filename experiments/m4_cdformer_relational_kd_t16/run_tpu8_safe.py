...
"""Process-isolated TPU8 launcher for CD-Former relational KD."""
...
'''
    result = subprocess.run(
        [sys.executable, "-c", code], env=forced_tpu_env(),
        capture_output=True, text=True, check=False,
    )
    combined = (result.stdout or "") + ("\n" + result.stderr if result.stderr else "")
    print("=" * 120, flush=True)
    print("M4-CD-FORMER-RELKD-T16 | ISOLATED TPU RUNTIME PROBE", flush=True)
    print("=" * 120, flush=True)
    ...
    if backend != "tpu" or count != EXPECTED_DEVICES:
        raise RuntimeError(f"Expected TPU8, got backend={backend!r}, count={count}")
    print("TPU_RUNTIME_PROBE=PASS | backend=tpu | local_devices=8", flush=True)
    print("=" * 120, flush=True)
...
def main() -> int:
    probe_tpu()
    run_child("experiments.m4_cdformer_relational_kd_t16.preflight_tpu8")

    print("PREFLIGHT PASSED — STARTING CD-FORMER RELATIONAL KD", flush=True)
    run_child(
        "experiments.m4_cdformer_relational_kd_t16.train_tpu",
        "--teacher",
        "/kaggle/input/models/romelbaldivia/cdformer-jax/jax/default/1/cdformer16_teacher_jax.npz",
        "--protocol", "both",
        "--epochs", "60",
        "--patience", "12",
        "--batch-size", "256",
        ...
        "--progress-every", "5",
        "--audit-first",
        "--outdir", "/kaggle/working/NestSAR_M4_CDFormer_RelKD_T16_TPU",
    )
    return 0
