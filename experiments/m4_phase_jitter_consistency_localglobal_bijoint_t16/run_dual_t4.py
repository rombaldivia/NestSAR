...
DEFAULT_OUTDIR = (
    "/kaggle/working/"
    "NestSAR_M4_LocalGlobal_BiJoint_T16_DualT4"
)
...
def worker_command(dataset: str, outdir: str, protocol: str) -> list[str]:
    return [
        ...
        "-u",
        "-m",
        "experiments.m4_phase_jitter_consistency_localglobal_bijoint_t16.train_gpu",
        "--dataset", dataset,
        "--protocol", protocol,
        "--epochs", "60",
        "--patience", "12",
...
def terminate_workers(processes: list[tuple[str, subprocess.Popen]]) -> None:
    ...
    while (
        time.time() < deadline
        and any(proc.poll() is None for _, proc in processes)
    ):
        time.sleep(0.2)
...
def main(
    ...
    print("=" * 120, flush=True)
    print(
        "NESTSAR LOCALGLOBAL V2 + BI-JOINT M4/G4 — FROM SCRATCH — KAGGLE DUAL T4",
        flush=True,
    )
    print("=" * 120, flush=True)
    ...
    if len(gpus) != EXPECTED_GPUS:
        raise RuntimeError(
            f"Expected exactly 2 GPUs from nvidia-smi, found {len(gpus)}"
        )
    ...
    print("=" * 120, flush=True)
    print("BI-JOINT MODEL/PARAM/XLA PREFLIGHT ON GPU0", flush=True)
    print("=" * 120, flush=True)
    ...
    if preflight.returncode != 0:
        raise RuntimeError(
            f"GPU preflight failed with exit code {preflight.returncode}"
        )
    ...
    if out_path.exists() and any(out_path.iterdir()):
        raise RuntimeError(
            ...
            "Move/delete it before a from-scratch rerun."
        )
    ...
    manifest = {
        "experiment": "M4LocalGlobalBiJointT16_DualT4",
        ...
        "patience": 12,
        "seed": 128,
        "optimization": "exact LocalGlobal V2 champion schedule",
        ...
        "architecture_change": "one-way joint GatedSweep -> BiMemory",
        "attention": False,
        "qkv": False,
    }

    (out_path / "dual_t4_manifest.json").write_text(
    ...
    try:
            ...
            if now - last_heartbeat >= 30.0:
                status = " | ".join(
                    ...
                    + (
                        "RUNNING"
                        if proc.poll() is None
                        else "DONE rc=" + str(proc.returncode)
                    )
                    for prefix, proc in processes
                )
                print(
                    f"[BIJOINT DUAL-T4 HEARTBEAT] elapsed={now-start:.0f}s | {status}",
                    flush=True,
                )
                last_heartbeat = now
        ...
        print(
            "[BIJOINT DUAL-T4] KeyboardInterrupt; stopping both workers...",
            flush=True,
        )
        terminate_workers(processes)
        print(f"Partial logs: {logs_dir}", flush=True)
        return 130
    ...
    if failures:
        raise RuntimeError(
            f"Dual-T4 worker failure(s): {failures}; logs={logs_dir}"
        )
    ...
    print("=" * 120, flush=True)
    print("BI-JOINT DUAL T4 DONE", flush=True)

    for protocol in ("xsub", "xset"):
        r = results[protocol]
        print(
            f"{protocol.upper()} "
            f"best={100*r['best_val_accuracy']:.6f}% @ E{r['best_epoch']} | "
            f"baseline={100*r['baseline_localglobal_v2']:.6f}% | "
