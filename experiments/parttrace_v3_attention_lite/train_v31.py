...
from experiments.parttrace_v3_attention_lite.model_v31 import make_wrapper_v31

EXPECTED_COUNTS = {"xsub": (63_026, 50_919), "xset": (54_468, 59_477)}


def parse_args():
    p = argparse.ArgumentParser(description="Train Attention-Lite + PartTrace v3.1 on one T4")
    ...
    # Score-driven differential learning rates.
...
def main() -> int:
    ...
    added = tp - bp
    if not (100_000 <= added <= 300_000):
        raise RuntimeError(f"V3.1 SIZE GUARD FAILED: added params={added:,}")
    ...
    print("NESTSAR ATTENTION-LITE + PARTTRACE V3.1 — SCORE-DRIVEN TRAINER", flush=True)
    print("=" * 118, flush=True)
    ...
    print(f"Aux CE={a.parttrace_aux_weight:g} predictive={a.predictive_loss_weight:g} controller_KL={a.controller_kl_weight:g}", flush=True)
    print(f"Branch influence frozen {a.freeze_branch_epochs} epoch(s), ramp={a.branch_ramp_epochs} epoch(s)", flush=True)
    print("=" * 118, flush=True)
    ...
    if a.audit_first:
        ...
        bf, ff = flops(base_comp), flops(full_comp)
        print(f"XLA base GFLOPs:     {bf/1e9:.9f}", flush=True)
        print(f"XLA v3.1 GFLOPs:     {ff/1e9:.9f}", flush=True)
        print(f"XLA added GFLOPs:    {(ff-bf)/1e9:.9f}", flush=True)
    ...
    for epoch in range(1, a.epochs + 1):
        ...
        sums = np.zeros(5, np.float64)  # loss/main/branch/predictive/KL
        ...
        for bi, (x_np, y_np) in enumerate(ns.batch_iterator(
            ...
            main_ce, branch_ce, predictive, controller_kl, acc, gate, eff_gate, weights = aux
            n = len(y_np)
            tr_count += n
        ...
        if improved:
            ...
            payload = {
                "model": "AttentionLitePartTraceV31",
                "protocol": a.protocol,
                "epoch": epoch,
                "seed": a.seed,
