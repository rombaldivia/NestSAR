...
def validate_config(config):
    ...
    if any(c[k] < 0 for k in ("weight_decay", "stream_aux_weight", "consistency_weight", "min_delta", "sm_residual_scale", "head_residual_scale")):
...
def ensure_runtime(out, bars, gpu):
    out = Path(out)
    probe = ("import jax,flax,optax,psutil,pytest,tqdm,numpy; "
...
def run(dataset=None, outdir="/kaggle/working/NestSAR_SM_ALL_T16_SharedCache_v2",
    ...
    try:
        ...
        gpus = gpus[:2]
        atomic_json(out / "hardware.json", {"nvidia_smi": nv, "assignment": dict(zip(("xsub", "xset"), gpus)), "pipeline_version": VERSION})
        bars = make_bars()
        ...
        source = subprocess.run(["git", "-C", str(Path(__file__).resolve().parents[3]), "rev-parse", "HEAD"],
                                capture_output=True, text=True)
        atomic_json(out / "source.json", {"commit": source.stdout.strip() if source.returncode == 0 else None,
        ...
        quiet_run([python, "-m", MODULE + ".data", "--dataset", str(dataset), "--cache", str(cache),
        ...
        if audit_first:
            quiet_run([python, "-m", MODULE + ".audit", "--config", str(cfg_path), "--output", str(out / "compute_audit.json")],
        ...
        for protocol, gpu in zip(("xsub", "xset"), gpus):
            ...
            cmd = [python, "-u", "-m", MODULE + ".worker", "--config", str(cfg_path), "--protocol", protocol,
