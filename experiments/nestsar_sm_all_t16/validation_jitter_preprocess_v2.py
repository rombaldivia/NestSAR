"""Frozen T16 segment-boundary views for the PREPROCESS_V2 SM-ALL checkpoints.
...
DEFAULT_OUT = "/kaggle/working/NestSAR_PREPROCESS_V2_T16_JITTER"
...
def make_views(raw: np.ndarray) -> dict[str, np.ndarray]:
    """Use the entire raw clip in each view; change only segment boundaries."""
    views = {"original": pp.features(raw),
...
def _refresh(output: Path, bars: list) -> None:
    for protocol, bar in zip(PROTOCOLS, bars):
        ...
        bar.set_postfix_str(" | ".join(f"{name}:{scores[name]*100:.2f}%"
                                     for name in ("original", "anchored") if name in scores), refresh=False)
        bar.refresh()
...
def launch(settings: dict | None = None) -> dict:
    ...
    print("One pickle-reading worker at a time to limit Kaggle RAM; no cache or model training.", flush=True)
        ...
        for gpu, protocol in enumerate(PROTOCOLS):
            ...
            with (output / protocol / "worker.log").open("w") as log:
                ...
                command = [sys.executable, "-u", "-m",
                           "experiments.nestsar_sm_all_t16.validation_jitter_preprocess_v2",
