...
checkpoint. It does not train, save weights, install packages, rebuild a cache,
or edit the raw skeleton. Both NTU120 protocols run on separate GPUs. An
existing compatible P2 cache and `best.msgpack` for each protocol are required.
...
two live tqdm rows. Each worker reads its EMA checkpoint; missing checkpoints
or mismatched preprocessing/cache signatures fail before inference. No training
