...
def main(dataset_path: str | None = None) -> None:
    ...
    for a in examples:
        ...
        local_coord_max = max(local_coord_max, float(np.max(np.abs(baseline_local - new_local))))
        old = phase.segment_phase_tokens(kp).reshape(phase.FRAMES, phase.PERSONS, phase.JOINTS, phase.TOKEN_CHANNELS)
        new = lg.segment_phase_tokens_localglobal(kp).reshape(phase.FRAMES, phase.PERSONS, phase.JOINTS, phase.TOKEN_CHANNELS)
        ...
        if new_local.shape[0] >= 2:
            root_motion_local.append(float(np.mean(np.abs(np.diff(new_local[:, 0, 0, :], axis=0)))))
    ...
    if root_motion_local:
        print("MEAN PERSON0 ROOT MOTION | LOCAL FRAMEWISE CENTERED:", float(np.mean(root_motion_local)), flush=True)
        print("MEAN PERSON0 ROOT MOTION | GLOBAL FIRST-ROOT FRAME:", float(np.mean(root_motion_global)), flush=True)
