...
def visible_gpus() -> list[str]:
    result = subprocess.run(["nvidia-smi", "--query-gpu=index,name,memory.total", "--format=csv,noheader"], capture_output=True, text=True, check=True)
    return [line.strip() for line in result.stdout.splitlines() if line.strip()]


def probe_gpu(index: int) -> None:
    code = "import jax; print('JAX_VERSION='+str(jax.__version__)); print('BACKEND='+str(jax.default_backend())); print('LOCAL_DEVICE_COUNT='+str(jax.local_device_count())); print('DEVICES='+repr(jax.local_devices()))"
    result = subprocess.run([sys.executable, "-c", code], env=gpu_env(index), capture_output=True, text=True, check=False)
