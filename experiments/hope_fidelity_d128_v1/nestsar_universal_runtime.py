...
TPU compatibility note
----------------------
Some exact audited v4.1 source revisions include the legacy --allow-cpu flag,
while others use the same non-GPU guard internally without exposing that CLI
flag. The universal runtime therefore removes --allow-cpu before delegating to
...
if BACKEND == "tpu":
    ...
    if removed_allow_cpu:
        sys.argv = [arg for arg in sys.argv if arg != "--allow-cpu"]

    _BASE_PARSE_ARGS = ns.parse_args

    def _universal_parse_args():
        args = _BASE_PARSE_ARGS()
        setattr(args, "allow_cpu", True)
        return args

    ns.parse_args = _universal_parse_args
    print(
        "TPU CLI compatibility: legacy --allow-cpu handled internally "
        f"(removed_from_argv={removed_allow_cpu})"
    )
...
print(
    f"RegMask:             frame={reg.FRAME_MASK_PROB:.3f} | "
    f"joint={reg.JOINT_MASK_PROB:.3f} | part={reg.PART_MASK_PROB:.3f}"
)
